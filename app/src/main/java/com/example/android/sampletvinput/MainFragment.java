...
import com.example.android.sampletvinput.data.FetchVideoService;
...
import com.example.android.sampletvinput.presenter.GridItemPresenter;
import com.example.android.sampletvinput.presenter.IconHeaderItemPresenter;
import com.example.android.sampletvinput.recommendation.UpdateRecommendationsService;
...
public class MainFragment extends BrowseFragment implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private static final int BACKGROUND_UPDATE_RELOAD = 30000;
...
//pridano diskCacheStrategy - kvuli vypnuti cache
        Glide.with(this)
                .load(uri)
                .asBitmap()
                .centerCrop()
        ...
        mBackgroundTimer.schedule(new UpdateBackgroundTask(), BACKGROUND_UPDATE_RELOAD);
...
// primy playback vybraneho videa
                Bundle bundle = ActivityOptionsCompat.makeSceneTransitionAnimation(
                        getActivity(),
                        ((ImageCardView) itemViewHolder.view).getMainImageView(),
                        VideoDetailsActivity.SHARED_ELEMENT_NAME).toBundle();
...
/*
    private void updateCard(ImageCardView cardView, String uri) {
